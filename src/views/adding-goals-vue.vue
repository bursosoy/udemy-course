...
<template>
  <div class="goal-main-wrap">
    <div class="cmn_card goal-card" :style="[!goals.length ? {margin: 'auto'} : {}]">
      <div>
        <a :href="stoicLink.link" target="_blank" v-html="stoicLink.label"></a>
      </div>
      <div>
        ...
        <button class="cmn_button btn_add" @click="addGoal(userInput)" :disabled="userInput === ''">
          Add
        </button>
      </div>
    </div>
    <div class="goal-list" v-if="goals.length">
      ...
      <div v-for="(goal, index) in goals" :key="goal" class="list-entry" :id="`goal${index}`" :class="{'list-entry-enabled': goal.isAboutToDelete}">
        <div class="userGoal" @click="displayisAboutToDelete(index)">
          ...
          <div v-if="goal.isAboutToDelete" class="btn-close" @click="removeGoal(index)" @click.stop>
            x
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      randomContent:
        'Go fishing in the Northwest Territories.Climb on a glacier in Alaska.Go on a safari in Africa.Become a certified scuba diver.Snorkel in Hawaii.Learn to sail.Skydive.Ride in a hot air balloon.Go on a river-rafting trip.Go kayaking.Attend Vic Braden’s tennis camp (Deleted due to ruptured disc).Fly in a glider.Dive in a submarine.Learn to windsurf.Learn to ski.Ski black diamond level (Deleted due to ruptured disc).Ski Vail, Aspen, Beaver Creek, Sun Valley & Jackson Hole.Ski in Europe (Deleted due to ruptured disc).Explore all 5 of Hawaii’s islands.Swim with the dolphins.Go to the Super Bowl.Go to the Rose Bowl.Go to the Final Four College Basketball Championships.Go to the Olympics.Take one of my sons to the Olympics',
    }
  },
  computed: {
    randomWords() {
  ...
  watch: {
    ...
    goals: {
      // deep and true for array. deep means an inside of an array was updated
      deep: true,
      handler() {
        console.log('watch: goals have been updated')
      },
    },
  },
  methods: {
